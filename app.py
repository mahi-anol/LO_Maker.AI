...
st.markdown("""
<style>
  .main-title { text-align:center; color:#2E75B6; font-size:2.2rem;
                font-weight:700; margin-bottom:0.2rem; }
  .sub-title  { text-align:center; color:#555; font-size:1rem;
                margin-bottom:1.5rem; }
  .info-box   { background:#f0f7ff; border-left:4px solid #2E75B6;
                padding:12px 16px; border-radius:4px; margin-bottom:1rem; }
  .embed-box  { background:#fff8e1; border-left:4px solid #f59e0b;
                padding:12px 16px; border-radius:4px; margin-bottom:1rem; }
  .stButton > button { background-color:#2E75B6; color:white;
                       font-size:1rem; font-weight:600; width:100%;
                       padding:0.55rem; border-radius:6px; border:none; }
  .stButton > button:hover { background-color:#1a5a9a; color:white; }
</style>
...
# ── API Key input ─────────────────────────────────────────────────────────────
if not get_api_key():
    st.warning("⚠️ OPENAI_API_KEY পাওয়া যাচ্ছে না।")
...
st.markdown("""
<div class="embed-box">
<b>Embedding কী?</b> পাঠ্যপুস্তক একবার প্রসেস করে সংরক্ষণ করলে পরের বার
...
embedding_mode = st.radio(
    "Embedding পদ্ধতি বেছে নিন:",
    options=[
        "🆕 PDF আপলোড করে নতুনভাবে তৈরি করো (প্রতিবার index করো)",
...
if use_saved:
    ...
    if not saved_aliases:
        st.warning("⚠️ কোনো সংরক্ষিত Embedding নেই। প্রথমে PDF আপলোড করে সংরক্ষণ করুন।")
        use_saved = False
    else:
        saved_alias_choice = st.selectbox(
            "সংরক্ষিত বই বেছে নিন:",
        ...
        # Delete option
        with st.expander("🗑️ সংরক্ষিত Embedding মুছে ফেলুন"):
    ...
    if save_embedding:
        new_alias = st.text_input(
            ...
            help="এই নামে Embedding সংরক্ষিত হবে। পরে এই নাম দিয়ে লোড করতে পারবেন।",
        )

st.divider()

# ── Model selection ───────────────────────────────────────────────────────────
st.subheader("⚙️ AI মডেল")
model_name = st.selectbox(
    "OpenAI মডেল বেছে নিন:",
...
# ══════════════════════════════════════════════════════════════════════════════
# Download Section (persistent — stays visible after generation)
# ══════════════════════════════════════════════════════════════════════════════
if st.session_state.docx_bytes is not None:
    ...
    # Clear button
    if st.button("🗑️ পরিষ্কার করুন (নতুন পাঠ পরিকল্পনা তৈরির জন্য)", key="clear_btn"):
        st.session_state.docx_bytes = None
        st.session_state.docx_filename = None
        st.session_state.lesson_plan = None
        st.rerun()
